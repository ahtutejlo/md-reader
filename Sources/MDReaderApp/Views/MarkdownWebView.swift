...
struct MarkdownWebView: NSViewRepresentable {
    ...
    <div id="content"></div>
    <script>
    // Note: all HTML here is generated by MarkdownRenderer from local file
    // content (not untrusted user input), so innerHTML assignment is safe.
    ...
    function _patchInPlace(oldEl, newEl) {
        if (oldEl.tagName === newEl.tagName) {
            // Same tag — transfer children to keep the DOM node alive and
            // avoid destroy/recreate which causes layout shift.
            if (oldEl.innerHTML !== newEl.innerHTML) {
                // Content is from MarkdownRenderer (local files only, not
